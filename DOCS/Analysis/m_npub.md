...
This document describes the `M_NPUB` layer, its purpose in S-57 charts, and how it is typically loaded, styled, and queried in a chart visualization application.
...
The rendering pipeline is typically defined in the application's S-52 styling configuration and implemented in the chart rendering logic:

### 3.1 Style Definition
In the S-52 styling configuration, `M_NPUB` is typically defined under `areaOutlines`:
...
The `CHGRY` color token is resolved dynamically at runtime using the active display mode. The hex colors used are defined in the S-52 color system documentation:
...
When the layer is loaded, the rendering code adds a `line` type layer to the map instance with the following layout and paint settings:
...
In S-52, chart features are grouped into display categories to help mariners declutter the screen. In the feature-grouping logic, `M_NPUB` is typically grouped into the `otherFeatures` set:
...
* **Geometry Rank**: Because the source data is a Polygon outlining the publication boundary, its geometry type places it in **Rank 4** (Polygons) during sorting in the rendering pipeline or priority **`39000`** in the viewing-group logic.
...
* **Min/Max Zoom**: `M_NPUB` is not listed in the layer zoom constraints configuration. Thus, its group minimum zoom defaults to `0`.
